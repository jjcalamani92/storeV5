...
import { ModalPageAntd } from "./formModal/formPageAntd";
...
interface HeadingDashboardPages {
  title: string;
  
  site:SiteV2
}
const formPage = [
  { name: 'Add Page', href: 'add-page', current: true },
  { name: 'Delete Pages', href: 'delete-pages', current: true },
...
export const HeadingDashboardPages: FC<HeadingDashboardPages> = ({ title, site }) => {
  ...
  const [openMI, setOpenMI] = useState(false)
  const [openMPD, setOpenMPD] = useState(false)
  const click = (data: string) => {
    if (data === 'add-page') {
      // console.log('update-product');
      setOpenMPD(true)
      setData(null)
    } else if (data === 'update-page') {
      setOpenMPD(true)
      setData(datas)
      // console.log('update-image');
  ...
  return (
    <div className="flex items-baseline justify-between py-6">
      ...
      <ModalPageAntd openMPD={openMPD} setOpenMPD={setOpenMPD} site={site} children={data!} /> 
    </div>
  )
}